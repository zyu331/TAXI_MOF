...
def pairInfoGenandDelete(framework):
    file1 = open('force_field_mixing_rules_4lmps.def', 'r')
    ...
    print('please check VDW order is consistent between force_field_mixing_rules_4lmps.def and .lmps')
    totalAtomType=int(Lines[5])
    f = open('data.pair','w')
...
def GenAdsorbateData(framework = None, adsorbate = None, cyclenumber = None):
    ...
    def adsorbate_write(NUM_adsorbate,restart_start):
        ...
        for i in range(4,len(keyOutput)):
            # if desiredOrder[i-4]!=keyOutput[i]:
            #     return("framwrok.lmps keyword order not correct, please change the order")
            adsorbate_lmp_output.write(Lines_ad[keywordDict_ad[desiredOrder[i-4]]])
            adsorbate_lmp_output.write('\n')
            for j in range(0,NUM_adsorbate):
                atomCount=0
                count_ad = keywordDict_ad[desiredOrder[i-4]]+2
                try:
                    while count_ad < keywordDict_ad[desiredOrder[i-3]]-1:
                        line_list=Lines_ad[count_ad].split()
                        line_list[0]= int( line_list[0]) + j*int(Lines_ad[2+i-4][0])
                        newline=''
                        if i==4:
                            for k in range(0,4):
    ...
    for i in range(0,2): # default to improper!
...
def write_raspa_input(temp=None, pressure=None, cyclenumber=None, cycles=None,framework=None, adsorbate=None, rosenbluth=None,HeliumVoidFraction=None,startNUM=None):
    if temp is None:
        print('you must specify a temperature')
        return
    if pressure is None:
...
read_data data."""+framework+""" extra/atom/types """+str(adsorbate_Types[0])+""" extra/bond/types """+str(adsorbate_Types[1])+""" extra/angle/types """+str(adsorbate_Types[2])+""" extra/dihedral/types """+str(adsorbate_Types[3])+""" extra/improper/types """+str(adsorbate_Types[4])+""" 
read_data data.adsorbate add append offset """+ str(f_Types[0])+' '+str(f_Types[1])+' '+str(f_Types[2])+' '+str(f_Types[3])+' '+str(f_Types[4])+"""
include   	 data.pair\n""")
...
############################### minimization ###########################################################
            ...
            fix		cell fram box/relax tri 0.0 vmax 0.001
            min_style	cg
            min_modify	line quadratic dmax 0.001
            minimize	1e-10 1e-10 10000 1000000
            unfix		cell
...
############################### NVT ###########################################################
            dump nvt_struc all xyz 50000 fig/nvt_framework_*.xyz
            ...
            undump nvt_struc\n\n
 ############################### NPT ###########################################################
            dump npt_struc all xyz 50000 fig/npt_framework_*.xyz
            dump_modify	npt_struc element """+" ".join(elementArray)+"""
            dump final fram custom 10000 fig/final_framework_* id mol type q x y z
...
def updatecif(f_lmps=None, f_cif=None, num_framework_atoms=None, cyclenumber=None):
    ...
    for line in f:
        ...
        if start_parsing == True and len(row) > 3 and int(row[0]) <= num_framework_atoms:
            pos_old=np.array([float(row[4]),float(row[5]),float(row[6])])
    ...
    for line in f:
        ...
        if row[0] == '_cell_length_a':
            ...
            fl.write(row[0]+' '+row[1]+'\t'+str(atoms[count][0])+'\t'+str(atoms[count][1])+'\t'+str(atoms[count][2])+'\t'+str(atoms[count][3])+'\n')
            count += 1
            continue
        fl.write(line)
    f.close()
    ...
    os.system('cp ' + f_cif+' '+str(cyclenumber)+'.cif')
    return
...
def updateFrameworkData(cyclenumber,framework):
    ...
    for line in f:
        ...
        if row[0]=='Atoms':
            origin_end=count2
        if row[0]=='Bonds':
            origin_bondStart=count2
            break	
    f.close()

    os.system('head -n '+str(origin_end+1)+' data.'+framework+'| tail -n '+str(origin_begin-origin_end-3)+' > output_property')
    os.system('head -n'+str(propertyEnd2-1)+ ' data.'+framework+' > output_head' )
    os.system('head -n '+str(result_begin-2)+' '+ str(cyclenumber)+'.lmps' + ' | tail -n ' +str(result_begin-propertyEnd-1)+ ' > output_cell')
    os.system('tail -n +10 fig/final_framework_500000 > output_pos')
    os.system('tail -n +'+str(origin_bondStart-1)+' data.'+framework+' > output_bonds')
    os.system('cat output_head output_cell > output_temp')
    os.system('cat output_temp output_property > output_temp2')
    os.system('cat output_temp2 output_pos > output_temp3')  
    os.system('cat output_temp3 output_bonds > output_final')  
    os.system('rm '+ 'data.'+framework)
