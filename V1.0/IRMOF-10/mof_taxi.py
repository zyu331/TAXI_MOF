                        ...
                        if add_atypes == True:
                                if count < 1:
                                        ...
                                        for i in adsorbate_angle_types:
                                                fl.write(str(int(i[0])+f_n_a_t)+' harmonic '+i[1]+'\t'+i[2]+'\n')
                                        add_atypes = False
                                        count = 0
                        if add_dtypes == True:
                                if count < 1:
                        ...
                        if add_itypes == True:
                                if count < 1:
                                        ...
                                        for i in adsorbate_improper_types:
                                                fl.write(str(int(i[0])+n_improper_types)+' '+i[1]+'\t'+i[2]+' '+i[3]+' '+i[4]+' '+i[5]+'\n')
                                        add_itypes = False
                                        count = 0
			fl.write('\n')
			continue
			...
			if int(row[0]) == f_n_at:
				for i in range(0, len(adsorbate_type_and_positions), 1):
					...
					fl.write(str(i+1+f_n_at) + ' ')
                                        ...
                                        lammps_mapping={'C_co2': '7', 'O_co2': '6'}
...
def write_raspa_input(temp=None, pressure=None, cyclenumber=None, cycles=None,framework=None, adsorbate=None, rosenbluth=None):
	...
	f.write('Movies                        yes\n')
	f.write('WriteMoviesEvery	10000\n\n')
...
def modify_output(cyclenumber):
	...
	os.system('mv '+str(cyclenumber)+'.lmps'+' yzz_temp1.lmps')
	os.system('head -n '+str(origin_end-1)+' temp_lmps.lmps'+'| tail -n '+str(origin_begin-origin_end-1)+' > yzz_temp2')
	os.system('head -n '+str(result_begin-2)+' '+ ' yzz_temp1.lmps' + ' > yzz_temp3')
	os.system('tail -n '+str(count-result_end+1)+' '+ ' yzz_temp1.lmps' + ' > yzz_temp4')
	os.system('cat yzz_temp3 yzz_temp2 > yzz_temp5')
	os.system('cat yzz_temp5 yzz_temp4 > yzz_temp6')
	os.system('mv yzz_temp6 '+lmpsName)
	os.system('rm yzz_temp*')	
	
def run_lammps(cyclenumber,prefix=None, np=None):
	if prefix is None:
